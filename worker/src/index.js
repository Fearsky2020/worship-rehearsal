...
Do not invent invisible text. Keep rejectionReason null when accepted.`;
...
function acceptVision(candidate,vision,type,meta){
 ...
 if(!vision.is_score)return vision.confidence<0.7&&metadataTitleStrong(candidate,meta);
...
async function searchType(env,meta,type,exclude,rejects,preferred,visionVerify=true,deadline=Date.now()+SEARCH_DEADLINE_MS){
 ...
 let clean=rows.filter(x=>x._score>=2&&!x.scoreReasons.includes("hard-video-host")&&!x.scoreReasons.includes("video-title"));
 if(clean.length<4)clean=rows.filter(x=>x._score>=0&&!x.scoreReasons.includes("hard-video-host"));
 ...
 for(const row of clean)if(accepted.length<3&&!seen.has(row))accepted.push({...row,verificationStatus:"heuristic-only"});
...
export default{async fetch(request,env){
 ...
 if(request.method==="POST"&&url.pathname==="/search")try{
   ...
   return json({schemaVersion:"1.5",title:meta.coreTitle,titleMeta:meta,results,queries,provider:"Brave Image Search + Workers AI Vision",requestsUsed,elapsedMs:Date.now()-startedAt,deadlineReached:Date.now()>=deadline,visionChecks,visionPassed,visionTimeouts,imageFetchTimeouts,gifConversions,hardVideoRejects,visionErrors:visionErrors.slice(0,6),visionModel:VISION_MODEL,filter:"visual-score-verification-v1"},200,origin)
